...
import {DownloadItem} from "@/types/hub";
...
interface ModelItemProps extends DownloadItem {
  variant?: 'default' | 'compact';
}

const ModelItem = memo<ModelItemProps>(
  ({ meta, createAt, model, homepage, identifier, variant }) => {
        ...
        <Flexbox className={styles.inner} gap={12}>
          ...
          {!isCompact && (
            <Flexbox gap={12} horizontal style={{ fontSize: 12 }}>
              <Link aria-label={model} href={homepage} target={'_blank'}>
                @{model}
              </Link>
